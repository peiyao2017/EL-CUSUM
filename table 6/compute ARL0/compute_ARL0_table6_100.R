...
threshold_ks=40
threshold_el_var=17
threshold_el_laplace=12
threshold_ael_var=1.3
threshold_ael_laplace=3.1
threshold_tel_var=7.8
threshold_tel_laplace=6 
threshold_tael_var=0.54 
threshold_tael_laplace=2.1
...
obs2=matrix(rmix(n=d*(maxobs-49),pii=c(0.5,0.5),family = "Skew.normal",arg = v0),nrow=maxobs-49,ncol=d)
obs=rbind(obs1,obs2)
...
for (n in (2+1): maxobs) {
  ...
  g_el_laplace[n] <- max(na.omit(c(lr,0)))
...
for (n in (d+1): maxobs) {
  ...
  g_ael_var[n] <- max(na.omit(c(lr,0)))
  print(n)
  print(length(lr))
...
for (n in (2+1): maxobs) {
  ...
  g_ael_laplace[n] <- max(na.omit(c(lr,0)))
...
for (n in (d+1): maxobs) {
  ...
  g_tel_var[n] <- max(na.omit(c(lr,0)))
...
for (n in (2+1): maxobs) {
  ...
  g_tel_laplace[n] <- max(na.omit(c(lr,0)))
...
for (n in (d+1): maxobs) {
  ...
  g_tael_var[n] <- max(na.omit(c(lr,0)))
  print(n)
  print(length(lr))
...
for (n in (2+1): maxobs) {
  ...
  g_tael_laplace[n] <- max(na.omit(c(lr,0)))
...
save(ARL0,file = "ARL0_Table6_100.RData")