setwd("/panfs/jay/groups/20/panwei/wan01299/EL_cusum/table5/")
library(doParallel)
...
threshold_ks=5
threshold_el_var=26
threshold_el_laplace=5.6
threshold_ael_var=2.8 
threshold_ael_laplace=2
threshold_tel_var=13
threshold_tel_laplace=2.9
threshold_tael_var=1.4
threshold_tael_laplace=1.6
...
results=foreach (i = 1:repetition, .combine='c', .multicombine=FALSE) %dopar% {
  ...
  train0=matrix(runif(n=d*m,min=-3,max=3),nrow=m,ncol=d)
  train1=matrix(runif(n=d*m,min=-3.5,max=3.5),nrow=m,ncol=d)
  ...
  obs1=matrix(runif(n=d*49,min=-3,max=3),nrow=49,ncol=d)
  obs2=matrix(runif(n=d*(maxobs-49),min=-3.5,max=3.5),nrow=maxobs-49,ncol=d)
  obs=rbind(obs1,obs2)
  ...
  lr =rowSums(dunif(x=obs ,min=-3.5,max=3.5,log=TRUE))-rowSums(dunif(x=obs ,min=-3,max=3,log = TRUE)) 
    lr=na.omit(lr)
...
save(ARL1,file = "ARL1_Table5_1000.RData")