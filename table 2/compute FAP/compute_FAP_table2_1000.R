...
threshold_tael_laplace=seq(from=1,to=3,by=0.1)
...
for(i1 in 1:length(threshold_opt)){
  ...
  if(d==1){
    lr = dmvnorm(x=obs ,mean=mu1,sigma = diag(Sigma,nrow=d,ncol=d),log = TRUE)-dmvnorm(x=obs[i,],mean=mu0,sigma = diag(Sigma,nrow=d,ncol=d),log = TRUE) 
  }
  if(d>1){
    lr = dmvnorm(x=obs ,mean=mu1,sigma = Sigma,log = TRUE)-dmvnorm(x=obs[i,],mean=mu0,sigma =Sigma,log = TRUE) 
  }
...
save(FAP,file = "FAP_Table2_1000.RData")